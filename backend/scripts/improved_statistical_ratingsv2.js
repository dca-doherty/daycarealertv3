...
const STATS_CONSTANTS = {
  ...
  // Explicit percentile-based thresholds to ensure distribution
  PERCENTILE_THRESHOLDS: {
    FIVE_STAR: 0.85,        // Top 15%
    FOUR_HALF_STAR: 0.70,   // Next 15%
    FOUR_STAR: 0.50,        // Next 20%
    THREE_HALF_STAR: 0.30,  // Next 20%
    THREE_STAR: 0.15,       // Next 15%
    TWO_HALF_STAR: 0.05,    // Next 10%
    TWO_STAR: 0.02,         // Next 3%
    ONE_HALF_STAR: 0.01     // Bottom 1%
  },
...
function calculateRatingFromPercentileAndRawScore(percentile, rawScore, dimensionType, reverseDirection = false) {
  ...
  if (thresholdMethod === 'percentile') {
    ...
    if (adjustedPercentile >= STATS_CONSTANTS.PERCENTILE_THRESHOLDS.FIVE_STAR) return 5.0;
    if (adjustedPercentile >= STATS_CONSTANTS.PERCENTILE_THRESHOLDS.FOUR_HALF_STAR) return 4.5;
    if (adjustedPercentile >= STATS_CONSTANTS.PERCENTILE_THRESHOLDS.FOUR_STAR) return 4.0;
    if (adjustedPercentile >= STATS_CONSTANTS.PERCENTILE_THRESHOLDS.THREE_HALF_STAR) return 3.5;
    if (adjustedPercentile >= STATS_CONSTANTS.PERCENTILE_THRESHOLDS.THREE_STAR) return 3.0;
    if (adjustedPercentile >= STATS_CONSTANTS.PERCENTILE_THRESHOLDS.TWO_HALF_STAR) return 2.5;
    if (adjustedPercentile >= STATS_CONSTANTS.PERCENTILE_THRESHOLDS.TWO_STAR) return 2.0;
    if (adjustedPercentile >= STATS_CONSTANTS.PERCENTILE_THRESHOLDS.ONE_HALF_STAR) return 1.5;
    return 1.0;
  } else {
    // Use raw score thresholds
    const thresholds = STATS_CONSTANTS.RAW_SCORE_THRESHOLDS[dimensionType];
    
    if (rawScore >= thresholds.FIVE_STAR) return 5.0;
    if (rawScore >= thresholds.FOUR_HALF_STAR) return 4.5;
    if (rawScore >= thresholds.FOUR_STAR) return 4.0;
    if (rawScore >= thresholds.THREE_HALF_STAR) return 3.5;
    if (rawScore >= thresholds.THREE_STAR) return 3.0;
    if (rawScore >= thresholds.TWO_HALF_STAR) return 2.5;
    if (rawScore >= thresholds.TWO_STAR) return 2.0;
    if (rawScore >= thresholds.ONE_HALF_STAR) return 1.5;
    return 1.0;
  }
}
