/**
 * API Data Updater for Production Server
...
const formatDate = (dateString) => {
  ...
  try {
    ...
    if (typeof dateString === 'string') {
      // ISO format: 2025-04-22T10:30:00.000Z
      if (dateString.includes('T')) {
        return dateString.split('T')[0];
      }
      // Simple date format: 2025-04-22
      if (dateString.includes('-') && dateString.length >= 10) {
        return dateString.substring(0, 10);
      }
    } else if (typeof dateString === 'number') {
      // Handle numerical timestamps (milliseconds since epoch)
    ...
    // Last resort: try to create a date object and format it
    const date = new Date(dateString);
    if (!isNaN(date.getTime())) {
      return date.toISOString().split('T')[0];
    }
    
    console.warn(`Unable to format date: ${typeof dateString} ${JSON.stringify(dateString)}`);
    return null;
  } catch (error) {
    console.error(`Error formatting date: ${typeof dateString} ${JSON.stringify(dateString)}`, error.message);
    return null;
  }
};
...
const formatDateTime = (dateTimeString) => {
  ...
  try {
    ...
    if (typeof dateTimeString === 'string') {
      // ISO format with T: 2025-04-22T10:30:00.000Z
      ...
      // Handle Date objects directly
      return dateTimeString.toISOString().replace('T', ' ').split('.')[0];
    }
    ...
    console.warn(`Unable to format datetime: ${typeof dateTimeString} ${JSON.stringify(dateTimeString)}`);
    return null;
  } catch (error) {
    console.error(`Error formatting datetime: ${typeof dateTimeString} ${JSON.stringify(dateTimeString)}`, error.message);
    return null;
  }
};
      ...
      for (const nonCompliance of batch) {
        try {
          ...
          if (activityResult.length > 0) {
            try {
              activityDate = activityResult[0].ACTIVITY_DATE;
            } catch (error) {
              console.error(`Error processing activity date for ${nonCompliance.activity_id}:`, error.message);
            }
          }
          ...
          try {
            correctedDate = formatDate(nonCompliance.corrected_date);
          } catch (error) {
            console.error(`Error processing corrected_date for ${nonComplianceId}:`, error.message);
          }
          
          try {
            dateVerified = formatDate(nonCompliance.date_correction_verified);
          } catch (error) {
            console.error(`Error processing date_correction_verified for ${nonComplianceId}:`, error.message);
          }
          ...
          const nonComplianceData = {
            ...
            DATE_CORRECTION_VERIFIED: dateVerified,
