/**
 * Generate Daycare Cost Estimation Model (Version 2)
 * 
 ...
 * 8. Additional economic indicators
 * 
 * Improvements in v2:
 * - Increased infant care premium to better reflect market reality
 * - Recalibrated capacity scaling to correctly reflect economies of scale
 * - Incorporated Zillow Home Value Index data for more accurate location-based pricing
 * - Added more advanced service detection and premium calculation
 * - Added a dedicated table for cost estimates instead of modifying daycare_operations
 */
...
// Base cost factors
const BASE_MONTHLY_COST = 850;  // Increased base monthly cost slightly

// Age-related cost multipliers - enhanced infant premium
const AGE_MULTIPLIERS = {
  infant: 1.7,      // Infants (0-17 months) cost 70% more (increased from 1.5)
  toddler: 1.35,    // Toddlers (18-35 months) cost 35% more (increased from 1.3)
...
const SERVICE_ADJUSTMENTS = {
  ...
  extendedHours: 12,        // Extended/overnight hours (increased from 10)
  meals: 7,                 // Meals provided
  specialNeeds: 18,         // Special needs accommodations (increased from 15)
  languageImmersion: 15,    // Language immersion programs (increased from 12)
  ...
  enrichmentPrograms: 10,   // Art/music/STEM programs (increased from 8)
...
const TYPE_MULTIPLIERS = {
  ...
  'Montessori': 1.35,       // Increased for premium Montessori programs
...
const RISK_ADJUSTMENTS = [
  { threshold: 70, discount: 18 },   // High risk (18% discount - increased from 15%)
  { threshold: 40, discount: 12 },   // Medium high risk (12% discount - increased from 10%)
  { threshold: 20, discount: 6 },    // Medium risk (6% discount - increased from 5%)
  { threshold: 10, discount: 0 },    // Low risk (no discount)
  { threshold: 0, premium: 6 }       // Very low risk (6% premium - increased from 5%)
];

// Experience-based adjustments
const EXPERIENCE_ADJUSTMENTS = [
  { years: 0, adjustment: -8 },    // New facilities (8% discount - increased from 5%)
  { years: 2, adjustment: 0 },     // 2-5 years (baseline)
  { years: 5, adjustment: 4 },     // 5-10 years (4% premium - increased from 3%)
  { years: 10, adjustment: 6 },    // 10-15 years (6% premium - increased from 5%)
  { years: 15, adjustment: 10 }    // 15+ years (10% premium - increased from 8%)
];

// County/city median income categories
const LOCATION_ADJUSTMENTS = {
  highIncome: 30,     // High income areas (>$100k median household income) - increased from 25%
  upperMiddle: 18,    // Upper middle income ($80k-$100k) - increased from 15%
  middle: 0,          // Middle income ($60k-$80k) - baseline
  lowerMiddle: -12,   // Lower middle income ($40k-$60k) - increased discount from 10%
  low: -25            // Low income areas (<$40k) - increased discount from 20%
};

// Capacity-based adjustments - recalibrated for economies of scale
function getCapacityAdjustment(capacity) {
  if (!capacity) return 0;
  
  if (capacity < 12) return 15;       // Small facilities (<12 children) - higher costs per child (increased from 10%)
  if (capacity < 25) return 8;        // Small-medium (12-24 children) - adjusted range
  if (capacity < 50) return 0;        // Medium (25-49 children) - baseline
  if (capacity < 100) return -8;      // Medium-large (50-99 children) - increased discount from 5%
  return -15;                         // Large facilities (100+ children) - increased discount from 10%
}
...
function getHoursAdjustment(hours, days) {
  ...
  if (hours && hours.toLowerCase().includes('24 hour')) {
    adjustment += 20;  // 24-hour care (increased from 15%)
  } else if (hours) {
...
async function loadZillowData() {
  try {
    ...
    const zhviData = {};
    const filePath = path.join('/home/dohertyb/daycarealert', 'Zip_zhvi_uc_sfrcondo_tier_0.33_0.67_sm_sa_month.csv');
...
// Load median incomes from public data and ZIP codes (as backup to Zillow data)
async function loadMedianIncomeData() {
  try {
    ...
    const texasCountiesIncomeData = {
      ...
      // Add the most relevant ZIP codes based on your data 
      zips: {
        ...
        '75093': { median_income: 125000, category: 'highIncome' },     // Plano/North Dallas
        '75205': { median_income: 135000, category: 'highIncome' },     // Highland Park, Dallas
        '75225': { median_income: 175000, category: 'highIncome' },     // Park Cities, Dallas
        '76092': { median_income: 175000, category: 'highIncome' },     // Southlake
        '77005': { median_income: 200000, category: 'highIncome' },     // West University, Houston
        '77019': { median_income: 130000, category: 'highIncome' },     // River Oaks, Houston 
        '77024': { median_income: 150000, category: 'highIncome' },     // Memorial, Houston
        '77401': { median_income: 145000, category: 'highIncome' },     // Bellaire, Houston
        '77479': { median_income: 140000, category: 'highIncome' },     // Sugar Land
        '77494': { median_income: 130000, category: 'highIncome' },     // Katy, Houston
        '78730': { median_income: 150000, category: 'highIncome' },     // Northwest Hills, Austin
        '78746': { median_income: 170000, category: 'highIncome' },     // West Lake Hills, Austin
        '78749': { median_income: 105000, category: 'highIncome' },     // Circle C, Austin
        '78681': { median_income: 97000, category: 'upperMiddle' },     // Round Rock
        '78759': { median_income: 95000, category: 'upperMiddle' }      // Northwest Austin
      }
    };
