...
function calculateRatingFromPercentileAndRawScore(percentile, rawScore, dimensionType, reverseDirection = false) {
  // SPECIAL HANDLING TO ENFORCE WHOLE NUMBER RATINGS
  // Force certain percentile ranges to get whole number ratings
  
  // Force top percentile band to get 5.0 stars
  if (percentile >= 0.95) {
    return 5.0;
  }
  
  // Force specific percentile bands to get whole number ratings
  if (percentile >= 0.65 && percentile < 0.70) {
    return 4.0; // Force 4.0 stars for this band
  }
  
  if (percentile >= 0.45 && percentile < 0.50) {
    return 3.0; // Force 3.0 stars for this band
  }
  
  if (percentile >= 0.25 && percentile < 0.30) {
    return 2.0; // Force 2.0 stars for this band
  }
  
  if (percentile <= 0.05) {
    return 1.0; // Force 1.0 stars for bottom percentiles
  }
  
  // Standard threshold method determination for remaining percentiles
