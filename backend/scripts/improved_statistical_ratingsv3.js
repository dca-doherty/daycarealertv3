...
const STATS_CONSTANTS = {
  ...
  PERCENTILE_THRESHOLDS: {
    ...
    ONE_HALF_STAR: 0.01,    // Next 1%
    ONE_STAR: 0.0           // Bottom 1%
  },
...
function determineThresholdMethod(percentile, rawScore, dimensionType) {
  ...
  // Add explicit handling for extremes - ensure top and bottom get proper ratings
  if (percentile >= 0.95) {
    return 'percentile'; // Guarantee 5-star ratings for top 5%
  }
  
  if (percentile <= 0.05) {
    return 'percentile'; // Guarantee bottom ratings for bottom 5%
  }
