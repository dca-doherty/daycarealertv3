    ...
    jwt.verify(token, process.env.JWT_SECRET, async (err, decodedToken) => {
      ...
      try {
        ...
        if (!user.verified && process.env.NODE_ENV !== 'development') {
          ...
          console.log('Development mode: bypassing verification check for user:', user.id);
        }
        ...
        if (process.env.NODE_ENV === 'development') {
          ...
          // If we can't find the mock user, use a default admin mock user
          console.log('Using default mock user (admin)');
