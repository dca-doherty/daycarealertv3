...
export const getFavorites = async () => {
  try {
    ...
    if (!token) {
      ...
      // Initialize empty favorites cache for non-logged in users
      window.favoritesCache = {};
      window.favoriteIds = [];
      return { success: true, message: 'User not logged in', favorites: [] };
    }
    ...
    // Initialize empty favorites cache on error
    window.favoritesCache = window.favoritesCache || {};
    window.favoriteIds = window.favoriteIds || [];
    return { success: false, message: error.response?.data?.message || 'Failed to fetch favorites', favorites: [] };
  }
};
